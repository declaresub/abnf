    ...
    def parse(self, source, start):
        ...
        for arg in self.args:
            try:
                ...
                raise ParseError('Error parsing %s at offset %s.' % (str(self),
                                 start)) from e
            else:
                nodes.append(node)
...
class Literal():  #pylint: disable=too-few-public-methods
    ...
    def __init__(self, value, case_sensitive=False):
        ...
        if not (isinstance(value, str) or (isinstance(value, tuple) and len(value) == 2 and isinstance(value[0], str) and isinstance(value[1], str))):
    ...
    def __str__(self):
        ...
        return "Literal(%s)" % str(value) if isinstance(self.value, tuple) else "Literal('%s'%s)" % (''.join(value), 'case_sensitive' if self.case_sensitive else '')
...
for grammar_rule_def in [
    ...
    ('repeat',
    Alternation(
        Concatenation(Repetition(Repeat(0, None), Rule('DIGIT')), Literal('*'), Repetition(Repeat(0, None), Rule('DIGIT'))),
