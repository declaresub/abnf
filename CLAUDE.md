...
- `tests/fuzz/` — Two independent corpora, both committed and replayed by CI:
  - `corpus/`, `corpus_neg/` — generated by `gen_corpus.py` (Hypothesis), which
    walks the *built combinators*
  - `corpus_abnfgen/` — generated by `gen_abnfgen_corpus.py`, which runs
    [abnfgen](http://www.quut.com/abnfgen/) over a module's grammar as *text*
    (reconstructed by `effective_grammar.py`).  Reading the text rather than
    the built parser is the point: it catches grammar that did not become the
    parser it describes.  Needs abnfgen on PATH, or `ABNFGEN` set, to
    regenerate; CI only replays.

  Regenerate both after changing a grammar module.
